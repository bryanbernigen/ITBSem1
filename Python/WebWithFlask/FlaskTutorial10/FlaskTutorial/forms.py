...
class RequestResetForm(FlaskForm):
    email = StringField('Email',
                        validators=[DataRequired(), Email()])
    submit = SubmitField('Kirim Permohonan')

    def validate_email(self, email):
        user = User.query.filter_by(email=email.data).first()
        if user is None:
            raise ValidationError('Tidak Ada Akun Dengan Email Tersebut. Mohon Daftar Terlebih Dahulu!.')
...
class ResetPasswordForm(FlaskForm):
    ...
    confirm_password = PasswordField('Konfirmasi Kata Sandi',
                                     validators=[DataRequired(), EqualTo('password')])
    submit = SubmitField('Atur Ulang Kata Sandi')
